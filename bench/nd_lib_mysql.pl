...
sub sql_next_in
{
    ...
    #
    # see nd_init_mysql.pl, st_id of zero must be illegal.
    #
    my $out_stream;
    my $first_done = 0;
...
sub sql_write_prep
{
    ...
    $sth = $dbh->prepare("select max(nd_pk) from nd_keys"); # currval('st_seq')");
    if ($dbh->err()) { die "$q_name 1\n$DBI::errstr\n"; }
    $sth->execute();
    if ($dbh->err()) { die "$q_name 2\n$DBI::errstr\n"; }
...
sub print_sf
{
    ...
    my $sql = "select st_id,is_active from stream_flags";
    my $sth = $dbh->prepare($sql);
    if ($dbh->err()) { die "$q_name 1\n$DBI::errstr\n"; }
    my $flag_rows = $sth->execute();
    if ($dbh->err()) { die "$q_name 2\n$DBI::errstr\n"; }
    my %sf;
    while ((my $st_id, my $is_active) = $sth->fetchrow_array())
    {
	print "s:$st_id a:$is_active\n";
    }
}
...
sub upstream_done
{
    ...
    my $flag_rows = $sth->execute();
    if ($dbh->err()) { die "$q_name 2\n$DBI::errstr\n"; }
    (my $is_active) = $sth->fetchrow_array();
    ...
    if (0)
    {
	print "$st_id dr:$data_rows fr:$flag_rows ia:$is_active\n";
	if ($data_rows == 0)
	{
	    print_sf($dbh, $st_id);
	}
