...
sub sql_init_streams
{
	...
	my $ct_sql = "create table stream_row_$st_id (
		st_id_fk integer,
		marked integer,
		data bytea
		) without oids;
		create index st_$st_id on stream_row_$st_id (st_id_fk);
		create index ma_$st_id on stream_row_$st_id (marked);
		create index stma_$st_id on stream_row_$st_id (st_id_fk,marked);";
	my $ct_sth = $dbh->prepare($ct_sql);
...
sub sql_next_in
{
	...
	if ($first_done)
	{
	    sleep(20); # sleep after the first try;
	}
