...
# Call sl_list to get HTML that is all the info for available search
# libraries.  Everything outside the body tags is stripped off, so
# this should work inside most web pages.  Returns a single string.
...
#
	...
	# Don't print all the fields in $hr, since they might
	# include all fields in bio_sequence. sl_link_url is
	# printed above differently because it is not in the $row
	# hash.
	# jhr added tab to align xml within output file (why? anal retentive maybe? )
	...
	# This is going into an XML file. XML has issues with
	# certain charcters in strings. No naked & allowed. Must
	# be &amp; There are probably many things that will break
	# the XML. It's a fragile technology.
...
# There are too many subs with _recs in the name. No naming convention
# based on function works, so just start naming subs with snappy catch
...
sub happy_recs
{
    ...
    {
	# Fun fact: text from textarea can have decimal 13 (^M aka
	# carriage return) chars on the end (and maybe in the
	# middle too) and chomp() doesn't get rid of them. This
	# fun fact brought to you by an hour of irritating
	# debugging.
	...
	# Getting sequence info from fastacmd (via dobyfasta()) is
	# less than optimal. We won't get a bs_pk, so some
	# downstream code may break in confusing ways.
...
# Normally, Perl is limited to 32 bit ints. Postgres allows 62 bits
# so use Math::BigInt to allow bitmasks up to 62 bits.
...
# Used by database code to get the
# name of the view used to access the blast_result table.
# Used in the cowpeaweb project.
...
# Get the view used by blast_result, and add a _bs suffix.
# Also see cowpeaweb/CGKB/cpweb_sql_lib.pl sql_create_view().
sub bs_view
{
    my $proto = $_[0];
    my $view = "";
...
#
# get sl_access bit setting value for comaprison against search_info
# requires the view name from current_ciew
#
...
# Use and eval for the log writing in case it fails. We still would
# like the stderr output from die.
...
sub err_stuff
{
    ...
    # Must save any necessary info from DBI before calling
    # write_log3() because write_log3() uses DBI to write log messages
    # to the db.
	...
	# Call write_log3() from an eval so that any failure in there
	# is not fatal, because we want to also die() after
	# logging. This really should either check that the db log was
	# written, or have an option to log to a file regardless. If
	# the db log fails, there is the die(), but if stderr is lost,
	# it would be helpful to have logging to a file (assuming that
	# the user will even see the file.)
...
sub db_info
{
    ...
    # In the simple case, no security is requried for our db info,
    # so just get the normal, non-hide values back. Non-secured db
    # info records should not have password, but we won't enforce
    # that here.

    if (! exists($cf{$db_alias}))
    {
	# We didn't find our db_alias in the non-hide section, so Here
	# we are asking only for a db config set of values. Getting
	# this required that $db_alias is in "hide", and has a config
	# value in db format (whitespace separated fields).
...
# twl8n sep 10 2008 Apache (or the CGI module) will only send a valid
...
sub process_template
{
    ...
    # aug 09 2008 twl8n. Regex excitement to support SSI includes. Use
...
# twl8n jun 5 2009: I can't see a reason for tags or keys. Table
